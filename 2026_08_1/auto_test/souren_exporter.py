...
class ResultExporter:
        ...
        try:
            ...
            for result in results:
                if 'loop_results' in result:
                    ...
                    for loop_result in workflow_result.get('loop_results', []):
                        merged = loop_result.copy()
                        ...
                        if 'start_time' in workflow_result:
                            merged['timestamp_readable'] = workflow_result.get('start_time', '')
                        if 'parameters' in workflow_result:
                            merged['parameters'] = workflow_result.get('parameters', {})
                        if 'device' not in merged:
                            merged['device'] = 'TCPIP0::192.168.30.122::inst0::INSTR'
                        if 'file' not in merged:
                            merged['file'] = workflow_result.get('script_name', '未知脚本')
                        all_test_results.append(merged)
                else:
            ...
            for config_name, config_results in config_groups.items():
                ...
                exec_time_str = first_result.get('timestamp_readable', first_result.get('timestamp', ''))
                if not exec_time_str and 'start_time' in first_result:
                    exec_time_str = first_result.get('start_time', 'N/A')
                if exec_time_str and 'T' in exec_time_str:
                    try:
                        dt = datetime.fromisoformat(exec_time_str.replace('Z', '+00:00'))
                        exec_time_str = dt.strftime('%Y-%m-%d %H:%M:%S')
                    except:
                        pass
                if not exec_time_str:
                    exec_time_str = 'N/A'

                script_file = first_result.get('file', first_result.get('script_name', ''))
                if isinstance(script_file, str):
                    if script_file.endswith('.py'):
                        script_file = script_file[:-3]
    ...
    def _set_axis_limits(self, ax, values):
        ...
        try:
            ...
            if min_val >= 0:
                y_lower = 0
                y_upper = np.ceil(max_val / tick_step) * tick_step
            elif max_val <= 0:
                y_lower = np.floor(min_val / tick_step) * tick_step
                y_upper = 0
            else:
                y_lower = np.floor(min_val / tick_step) * tick_step
                y_upper = np.ceil(max_val / tick_step) * tick_step

            if max_val > 0 and y_upper - max_val < tick_step * 0.6:
                y_upper += tick_step
            if min_val < 0 and min_val - y_lower < tick_step * 0.6:
                y_lower -= tick_step
    ...
    def _create_charts_sheet(self, writer: pd.ExcelWriter, results: List[Dict]):
        ...
        for cfg_name, cmd_title_dict in config_groups.items():
            for group_key, items in cmd_title_dict.items():
                ...
                if items and all(it.get('x_label') is not None for it in items):
                    unique_x_labels = set(it['x_label'] for it in items)
                    unique_loops = set(it['loop_iter'] for it in items)
                    if len(unique_x_labels) == 1 and len(unique_loops) > 1:
                        for it in items:
                            it['x_label'] = f"{it['x_label']} (循环{it['loop_iter']})"
                ...
                if separate:
                    ...
                    # 横坐标带 [失败] 的档位标红(柱子+数值)，与正常档位区分
                    fail_flags = ['[失败]' in str(lbl) for lbl in x_labels_raw]
                    bar_colors = ['crimson' if f else 'steelblue' for f in fail_flags]
                    ...
                    bar_width = 0.8
                    ax.bar(x_pos, values, width=bar_width, alpha=bar_alpha, edgecolor='black', linewidth=0.5, color=bar_colors)

                    self._set_axis_limits(ax, values)

                    for i, val in enumerate(values):
                        ax.text(i, val, f"{val:.3f}",
                                ha='center', va='bottom' if val >= 0 else 'top',
                                fontsize=7, fontweight='bold', color='red' if fail_flags[i] else 'black')
                    ...
                    if all_have_xlabel:
                        ...
                        self._set_axis_limits(ax, all_values)
...
def create_summary_excel(main_execution_dir: str, all_results: List[Dict] = None) -> Optional[str]:
    """
    创建图表汇总Excel文件，配置顺序严格按照 all_results（即执行顺序）排列。
    """
    try:
        print(f"\n{'='*60}")
        ...
        ordered_configs = []
        rate_map = {}  # (config_name, loop_iter) -> success_rate

        # 优先从 all_results（内存数据）构建 ordered_configs 和 rate_map
        if all_results:
            for result in all_results:
                # 优先使用 script_directory 作为配置名称（与实际目录名完全一致）
                script_dir = result.get('script_directory')
                if script_dir:
                    config_name = os.path.basename(script_dir)
                else:
                    config_name = exporter._get_config_name(result)
                if config_name not in ordered_configs:
                    ordered_configs.append(config_name)
                    print(f"   📁 提取配置名称: {config_name}")

                loop_results = result.get('loop_results', [])
                for loop in loop_results:
                    loop_idx = loop.get('loop_index', 1)
                    result_data = loop.get('result', {})
                    rate = result_data.get('success_rate')
                    if rate is not None:
                        rate_map[(config_name, loop_idx)] = rate

        # 如果没有 all_results，则从文件系统扫描（此时无法保证顺序，但尽量按字母）
        if not ordered_configs:
            dirs = [d for d in os.listdir(main_execution_dir) if os.path.isdir(os.path.join(main_execution_dir, d)) and not d.startswith('_')]
            ordered_configs = sorted(dirs)

        # 从文件系统提取 extracted_data（图表数据），按照 ordered_configs 的顺序
        all_extracted = []
        for config_name in ordered_configs:
            config_dir = os.path.join(main_execution_dir, config_name)
            if not os.path.isdir(config_dir):
                continue
            json_files = []
            for file in os.listdir(config_dir):
                if file.endswith('.json') and not file.startswith('summary'):
                    json_files.append(os.path.join(config_dir, file))
            if not json_files:
                print(f"⚠️  配置 {config_name} 中没有找到 JSON 结果文件，跳过")
                continue
            ...
            try:
                ...
                total_extracted = 0
                for entry in data:
                    extract_list = _extract_extracted_data_from_json(entry, config_name)
                    all_extracted.extend(extract_list)
                    total_extracted += len(extract_list)
                    
                    # 提取成功率（如果 rate_map 还没有）
                    if (config_name, 1) not in rate_map:
                        if 'loop_results' in entry and isinstance(entry['loop_results'], list):
                            for loop in entry['loop_results']:
                                loop_idx = loop.get('loop_index', 1)
                                result_data = loop.get('result', {})
                                rate = result_data.get('success_rate')
                                if rate is not None:
                            ...
                            loop_idx = entry.get('loop_iteration', 1)
                            if rate is not None:
                                rate_map[(config_name, loop_idx)] = rate
                
                print(f"✅ 从 {config_name} 提取到 {total_extracted} 个数据点")
            except Exception as e:
        ...
        # 去重：每个循环只保留最后一个值（即最终值）
        grouped = defaultdict(list)
        for item in all_extracted:
            key = (item['config_name'], item['command'], item.get('chart_title'), item['loop_iter'])
            grouped[key].append(item)
        all_extracted = []
        for group in grouped.values():
            group.sort(key=lambda x: x['seq_in_loop'])
            all_extracted.append(group[-1])   
        print(f"📊 按循环保留最终值后，数据点精简为 {len(all_extracted)} 条")
        ...
        _populate_chart_summary_sheet(ws_summary, all_extracted, ordered_configs)

        ws_comparison = wb.create_sheet("横向对比")
        _populate_comparison_sheet(ws_comparison, all_extracted, ordered_configs)

        ws_table = wb.create_sheet("汇总表格")
        _populate_summary_table_sheet(ws_table, all_extracted, ordered_configs)

        ws_final = wb.create_sheet("最终数据汇总")
        _populate_final_data_summary(ws_final, all_extracted, ordered_configs, rate_map)

        wb.save(summary_file)
        print(f"\n{'='*60}")
        print(f"✅ 图表汇总Excel文件创建成功!")
        print(f"📁 文件路径: {summary_file}")
        print(f"📊 工作表: 图表汇总, 横向对比, 汇总表格, 最终数据汇总")
        print(f"{'='*60}")
...
def _set_axis_limits_global(ax, values):
    if values is None:
        return
    try:
        import numpy as np
        ...
        # 顶部/底部留白：柱端数值标签需要空间，紧贴刻度边界时再抬/降一格
        if max_val > 0 and y_upper - max_val < tick_step * 0.6:
            y_upper += tick_step
        if min_val < 0 and min_val - y_lower < tick_step * 0.6:
            y_lower -= tick_step
        ...
        print(f"设置全局Y轴范围时出错: {e}")
        pass


def _populate_chart_summary_sheet(ws, all_extracted, ordered_configs=None):
    if not all_extracted:
    ...
    if ordered_configs:
        sorted_configs = [cfg for cfg in ordered_configs if cfg in config_groups]
        extra_configs = [cfg for cfg in config_groups.keys() if cfg not in ordered_configs]
        sorted_configs.extend(extra_configs)
    else:
    ...
    for cfg_name in sorted_configs:
        ...
        for group_key, items in cmd_title_dict.items():
            ...
            if separate:
                ...
                _set_axis_limits_global(ax, values)
                ...
                if all_have_xlabel:
                    ...
                    _set_axis_limits_global(ax, all_values)
...
def _populate_comparison_sheet(ws, all_extracted, ordered_configs=None):
    ...
    for (cmd, title), config_items in comparison_items.items():
        cfg_names = list(config_items.keys())
        if ordered_configs:
            sorted_cfg_names = [cfg for cfg in ordered_configs if cfg in cfg_names]
            extra_cfgs = [cfg for cfg in cfg_names if cfg not in ordered_configs]
            sorted_cfg_names.extend(extra_cfgs)
        else:
            sorted_cfg_names = sorted(cfg_names)

        config_data = {}
        all_loops = set()
        for cfg in sorted_cfg_names:
        ...
        n_configs = len(sorted_cfg_names)
        if n_configs >= 8:
            label_fontsize = 7
            rotation = 60
        elif n_configs >= 5:
        ...
        config_centers = [cfg_start_x[cfg] + cfg_widths[cfg]/2 for cfg in sorted_cfg_names]
        ax.set_xticks(config_centers)
        ax.set_xticklabels(sorted_cfg_names, rotation=rotation, ha='right', fontsize=label_fontsize)
        ...
        _set_axis_limits_global(ax, all_vals)
        ...
        ws.cell(row=info_row + 1, column=1, value=f"配置数量: {len(sorted_cfg_names)}")
...
def _populate_summary_table_sheet(ws, all_extracted, ordered_configs=None):
    cmd_title_groups = defaultdict(lambda: defaultdict(list))
    for item in all_extracted:
        cmd = item['command']
        cfg = item['config_name']
    ...
    if not comparison_items:
        ws.cell(row=1, column=1, value="无横向对比数据，无法生成汇总表格")
        print("   ⚠️ 汇总表格无数据")
        return
    ...
    for (cmd, title), config_items in comparison_items.items():
        title_cell = ws.cell(row=current_row, column=1, value=f"命令: {title}")
        title_cell.font = Font(bold=True, size=12)
        current_row += 1

        config_names = list(config_items.keys())
        if ordered_configs:
            sorted_configs = [cfg for cfg in ordered_configs if cfg in config_names]
            extra_configs = [cfg for cfg in config_names if cfg not in ordered_configs]
            sorted_configs.extend(extra_configs)
        else:
            sorted_configs = sorted(config_names)
        ...
        for cfg, items in config_items.items():
            ...
            for it in items:
                loop = it['loop_iter']
                val = it['value']
                if loop not in loop_dict:
                    loop_dict[loop] = val
                all_loops.add(loop)
            config_data[cfg] = loop_dict

        loops = sorted(all_loops)

        headers = ["配置名称"]
        for loop in loops:
            headers.append(f"第{loop}次循环")
        ...
        for cfg in sorted_configs:
            row_data = [cfg]
            for loop in loops:
                val = config_data[cfg].get(loop, "")
                if val != "" and val is not None:
                    try:
                        row_data.append(f"{float(val):.3f}")
                    except:
    ...
    max_loop_cols = max((len(headers)-1 for _, config_items in comparison_items.items()), default=0)
    for col in range(2, 2 + max_loop_cols):
        col_letter = get_column_letter(col)
        ws.column_dimensions[col_letter].width = 12

    print(f"   ✅ 汇总表格工作表已生成，共 {len(comparison_items)} 个命令表格")


def _populate_final_data_summary(ws, all_extracted, ordered_configs=None, rate_map=None):
    ...
    NR_METRIC_TITLES = ["DL BLER", "UL BLER", "NR TXP AVG"]
    LTE_METRIC_TITLES = ["DL LTE_BLER", "UL LTE_BLER", "LTE TXP AVG"]
    NR_TITLES = set(NR_METRIC_TITLES)
    LTE_TITLES = set(LTE_METRIC_TITLES)

    def normalize_title(title):
        if title in TITLE_MERGE_MAP:
            return TITLE_MERGE_MAP[title]
        return title
    ...
    config_loop_data = defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: None)))
    config_items = defaultdict(list)
    for item in all_extracted:
        config_items[item['config_name']].append(item)
    ...
    for cfg, items in config_items.items():
        ...
        for loop_iter, loop_items in loop_groups.items():
            loop_items.sort(key=lambda x: x.get('seq_in_loop', 0))
            title_groups = defaultdict(list)
            for it in loop_items:
                raw_title = it.get('chart_title')
                if raw_title:
                    norm_title = normalize_title(raw_title)
                    title_groups[norm_title].append(it)
            for norm_title, group in title_groups.items():
                config_loop_data[cfg][loop_iter][norm_title] = group[-1]

    if not config_loop_data:
        ws.cell(row=1, column=1, value="无法生成最终数据汇总")
        return

    all_titles = set()
    all_loops = set()
    for cfg, loops in config_loop_data.items():
        all_loops.update(loops.keys())
        for loop_iter, title_dict in loops.items():
            all_titles.update(title_dict.keys())

    present_nr_titles = [t for t in NR_METRIC_TITLES if t in all_titles]
    present_lte_titles = [t for t in LTE_METRIC_TITLES if t in all_titles]
    extra_titles = sorted(t for t in all_titles if t not in NR_TITLES and t not in LTE_TITLES)

    # 按 ordered_configs 排序
    if ordered_configs:
        sorted_configs = [cfg for cfg in ordered_configs if cfg in config_loop_data]
        extra = [cfg for cfg in config_loop_data if cfg not in ordered_configs]
        sorted_configs.extend(extra)  # 额外配置放在最后
    else:
        sorted_configs = sorted(config_loop_data.keys())
    ...
    # 停止功率点的状态显示：绿=正常边界，红=回退后异常
    STATUS_TEXT = {"normal": "正常", "abnormal": "回退后异常"}
    STATUS_FONT = {
        "normal": Font(color="008000", bold=True),
        "abnormal": Font(color="FF0000", bold=True),
    }

    # 表头：配置名称、循环次数、本循环成功率(%)、NR功率+NR指标、LTE功率+LTE指标、其他指标
    headers = (["配置名称", "循环次数", "本循环成功率(%)", "NR功率(dBm)"] + present_nr_titles
               + ["LTE功率(dBm)"] + present_lte_titles + extra_titles)
    current_row = 1
    for col_idx, header in enumerate(headers, start=1):
        cell = ws.cell(row=current_row, column=col_idx, value=header)
        cell.fill = header_fill
    ...
    def format_metric(title_dict, title):
        if title in title_dict:
            val = title_dict[title]['value']
            try:
    ...
    for cfg in sorted_configs:
        loops_in_cfg = sorted(config_loop_data[cfg].keys())
        for loop_iter in loops_in_cfg:
            title_dict = config_loop_data[cfg][loop_iter]

            # 本循环成功率
            rate = rate_map.get((cfg, loop_iter), "") if rate_map else ""
            if isinstance(rate, (int, float)):
                rate_str = f"{rate:.2f}"
            else:
                rate_str = str(rate) if rate else ""

            # 分别提取 NR 和 LTE 的功率 (x_label) 及其停止状态
            nr_power_val = ""
            lte_power_val = ""
            nr_status = None
            lte_status = None
            for title, last_item in title_dict.items():
                if not last_item or last_item.get('x_label') is None:
                    continue
                st = last_item.get('status')
                if title in NR_TITLES:
                    if st or not nr_power_val:
                        nr_power_val = str(last_item['x_label'])
                    if st and not nr_status:
                        nr_status = st
                elif title in LTE_TITLES:
                    if st or not lte_power_val:
                        lte_power_val = str(last_item['x_label'])
                    if st and not lte_status:
                        lte_status = st

            # 功率单元格文本追加中文状态，如 -88(正常) / -78(回退后异常)
            nr_power_text = nr_power_val
            if nr_power_val and nr_status in STATUS_TEXT:
                nr_power_text = f"{nr_power_val}({STATUS_TEXT[nr_status]})"
            lte_power_text = lte_power_val
            if lte_power_val and lte_status in STATUS_TEXT:
                lte_power_text = f"{lte_power_val}({STATUS_TEXT[lte_status]})"

            row_data = [cfg, loop_iter, rate_str, nr_power_text]
            row_data.extend(format_metric(title_dict, t) for t in present_nr_titles)
            row_data.append(lte_power_text)
            row_data.extend(format_metric(title_dict, t) for t in present_lte_titles)
            row_data.extend(format_metric(title_dict, t) for t in extra_titles)

            nr_power_col = 4
            lte_power_col = 4 + len(present_nr_titles) + 1
            for col_idx, value in enumerate(row_data, start=1):
                cell = ws.cell(row=current_row, column=col_idx, value=value)
                cell.border = thin_border
                cell.alignment = left_align if col_idx == 1 else center_align
                if col_idx == nr_power_col and nr_status in STATUS_FONT:
                    cell.font = STATUS_FONT[nr_status]
                elif col_idx == lte_power_col and lte_status in STATUS_FONT:
                    cell.font = STATUS_FONT[lte_status]
            current_row += 1
    ...
    nr_block_end = 4 + len(present_nr_titles)
    for i, title in enumerate(present_nr_titles, start=5):
        col_letter = get_column_letter(i)
        ws.column_dimensions[col_letter].width = max(15, len(title) + 4)
    ws.column_dimensions[get_column_letter(nr_block_end + 1)].width = 15
    for i, title in enumerate(present_lte_titles, start=nr_block_end + 2):
        col_letter = get_column_letter(i)
        ws.column_dimensions[col_letter].width = max(15, len(title) + 4)
    lte_block_end = nr_block_end + 1 + len(present_lte_titles)
    for i, title in enumerate(extra_titles, start=lte_block_end + 1):
        col_letter = get_column_letter(i)
        ws.column_dimensions[col_letter].width = max(15, len(title) + 4)

    print(f"   ✅ 最终数据汇总工作表已生成，包含 {len(sorted_configs)} 个配置，每个循环一行，含本循环成功率、NR/LTE功率和关键指标")
