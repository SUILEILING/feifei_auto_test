...
class SourenEngine:
        ...
        try:
            ...
            if success:
                ...
                if result.get("interrupted"):
                    ...
                    final_result["file"] = current_file_name
                    final_result["parameters"] = self.params
                    final_result["error"] = result.get("error", "用户中断")
                    return final_result
                else:
                    return {"success": False, "error": result.get("error", "执行脚本失败"),
                            "loop_iteration": loop_iteration, "loop_count": loop_count}
        except KeyboardInterrupt:
            print("\n⏹️ 测试执行被用户中断，正在保存已执行数据...")
            execution_details = getattr(self.script_executor, 'execution_details', [])
            extracted_data = getattr(self.script_executor, 'extracted_data', [])
            self.extracted_data = extracted_data
            final_result = self._generate_final_result(
                execution_details, device_name, "normal", loop_iteration, loop_count
            )
    ...
    def _generate_final_result(self, execution_details: List, device_name: str, mode: str,
        ...
        result = {
            ...
            "start_time": datetime.fromtimestamp(self.execution_start_time).strftime('%Y-%m-%d %H:%M:%S') if self.execution_start_time else "",   # 修改
            "end_time": datetime.now().strftime('%Y-%m-%d %H:%M:%S'),   # 修改
