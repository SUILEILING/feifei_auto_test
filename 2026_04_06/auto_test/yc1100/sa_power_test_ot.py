...
def case_body():
    ...
    def run_power_scan(start_power, end_power, step, fallback_delta, ap):   
        for _ in range(3):
            ...
            while True:
                ...
                dl_bler_str = ap.send("FETCh:NR:BLER:DL:RESult?", 7, True, "DL BLER", f"{current_power}",True)
                ul_bler_str = ap.send("FETCh:NR:BLER:UL:RESult?", 7, True, "UL BLER", f"{current_power}",True)
                dl_bler=float(dl_bler_str.split(',')[7])
                ul_bler=float(ul_bler_str.split(',')[7])
                UE_status = ap.send("CONFigure:CELL1:NR:SIGN:UE:STATe?")

                print(f"功率: {current_power} dBm, UE状态: {UE_status}, DL BLER: {dl_bler}  UL BLER: {ul_bler}")

                is_failure = (dl_bler > 0.2) or (ul_bler > 0.2) or (UE_status != '"Connected"')
                ...
                if is_failure:
                    ...
                    dl_bler_retry_str = ap.send("FETCh:NR:BLER:DL:RESult?", 7, True, "DL BLER", f"{new_power}",True)
                    dl_bler_retry=float(dl_bler_retry_str.split(',')[7])
                    ul_bler_retry_str = ap.send("FETCh:NR:BLER:UL:RESult?", 7, True, "UL BLER", f"{new_power}",True)
                    ul_bler_retry=float(ul_bler_retry_str.split(',')[7])

                    UE_status_retry = ap.send("CONFigure:CELL1:NR:SIGN:UE:STATe?")

                    is_failure_retry = (dl_bler_retry > 0.2) or (ul_bler_retry > 0.2) or (UE_status_retry != '"Connected"')

                    if is_failure_retry:
                        if new_power in failed_powers:
                            print(f"❌ 回退后功率 {new_power} dBm 已经失败过，停止所有测试")
                        else:
                            failed_powers.add(new_power)
                            print(f"❌ 回退后仍失败，停止所有测试")
                        return True  # 完全停止
                    else:
    ...
    should_stop = run_power_scan(start_power, end_power, step, fallback_delta, ap)
    if should_stop:
        print("测试因连续失败而终止")
