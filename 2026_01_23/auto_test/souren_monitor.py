...
class SourenEngine:
    ...
    def _run_tests_with_souren(self, mode: str = 'run_all', loop_count: int = 1) -> Dict:
        ...
        try:
            ...
            # ⭐⭐⭐ 关键修改：获取需要跳过的步骤命令（从当前Python文件中）⭐⭐⭐
            skip_commands = set()
            
            try:
                # 直接读取当前文件（假设是Python文件）
                print(f"\n📖 读取文件: {current_file_name}")
                with open(self.current_file_path, 'r', encoding='utf-8') as f:
                    content = f.read()
                
                # 查找 SKIP_IN_NEXT_CYCLES 定义
                import re
                
                # 查找SKIP_IN_NEXT_CYCLES列表定义
                # 匹配格式: SKIP_IN_NEXT_CYCLES = [...]
                skip_pattern = r'SKIP_IN_NEXT_CYCLES\s*[:=]\s*\[(.*?)\]'
                skip_match = re.search(skip_pattern, content, re.DOTALL)
                
                if skip_match:
                    skip_content = skip_match.group(1)
                    print(f"✅ 找到SKIP_IN_NEXT_CYCLES定义")
                    
                    # 提取所有字典中的command字段
                    # 匹配格式: "command": "具体命令"
                    command_pattern = r'"command"\s*:\s*"([^"]+)"'
                    found_commands = re.findall(command_pattern, skip_content)
                    
                    if found_commands:
                        skip_commands = set(found_commands)
                        print(f"📋 提取到 {len(skip_commands)} 个需要跳过的命令")
                        ...
                        print(f"⚠️  在SKIP_IN_NEXT_CYCLES定义中未找到command字段")
                        print(f"   SKIP_IN_NEXT_CYCLES内容预览:")
                        print(f"   {skip_content[:500]}...")
                else:
                    print(f"⚠️  未找到SKIP_IN_NEXT_CYCLES定义")
            ...
            # 如果是Python文件，直接从中提取测试步骤
            test_steps = []
            if self.current_file_path.endswith('.py') and 'TEST_STEPS' in content:
                print(f"📝 从Python文件中提取TEST_STEPS...")
                # 查找TEST_STEPS列表定义
                test_steps_pattern = r'TEST_STEPS\s*[:=]\s*\[(.*?)\]'
                test_steps_match = re.search(test_steps_pattern, content, re.DOTALL)
                
                if test_steps_match:
                    test_steps_content = test_steps_match.group(1)
                    # 提取所有命令
                    command_pattern = r'"command"\s*:\s*"([^"]+)"'
                    commands = re.findall(command_pattern, test_steps_content)
                    ...
                    print(f"✅ 从Python文件中提取 {total_steps} 个测试步骤")
                else:
                    print(f"⚠️  未找到TEST_STEPS定义，尝试解析为普通文件")
            ...
            try:
                if mode == 'loop_info' and loop_count > 1:
                    ...
                    for loop_index in range(loop_count):
                        ...
                        # ⭐⭐⭐ 关键修改：根据循环索引处理跳过逻辑 ⭐⭐⭐
                        steps_to_execute = steps  # 默认执行所有步骤
