...
# ⭐⭐⭐ SCV 文件配置选项 ⭐⭐⭐
# ==============================================
SCV_FOLDER = r"save_scv"  # SCV文件目录
DEFAULT_SCV_NAME = "night_cycle_update_feifei.scv"  # 默认SCV文件

# SCV生成选项
# 如果为True，则使用python脚本动态生成SCV
# 如果为False，则使用现有的SCV文件
...
LOOP_COUNT = 1  # 只有loop_info模式才能使用
...
EXECUTION_DIR = os.path.join(LOG_DIR, f"execution_{timestamp}")

if not os.path.exists(EXECUTION_DIR):
    os.makedirs(EXECUTION_DIR)

LOG_FILE = os.path.join(EXECUTION_DIR, f"souren_execution_{timestamp}.log")
RESULT_FILE = os.path.join(EXECUTION_DIR, f"souren_results_{timestamp}.json")
