...
class SourenEngine:
    ...
    def _select_and_connect_device(self) -> Dict:
        ...
        try:
            ...
            # 使用DirectCommandExecutor已经连接，这里只需记录设备信息
            device_info = {
                "id": device_address,
                "name": device_address,
                "type": "TCPIP",
    ...
    def _run_tests_with_souren(self, mode: str = 'run_all', loop_count: int = 1) -> Dict:
        ...
        try:
            ...
            if analysis:
                ...
                if analysis.get('estimated_duration_sec', 0) > 0:
                    print(f"   预计耗时: {analysis.get('estimated_duration_sec', 0):.1f}秒")
            
            # 计算总执行次数
            total_executions = total_steps
            if mode == 'loop_info' and loop_count > 1:
                total_executions = total_steps * loop_count
                print(f"\n🔄 开始执行{total_executions}个测试步骤 ({total_steps}步骤 × {loop_count}次全局循环)...")
            else:
            ...
            if should_stop:
                if stop_reason == "用户取消":
                    ...
                    message = f"测试被用户停止 (设备: {device_name})"
