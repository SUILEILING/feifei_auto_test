...
def main():
    ...
    if args.py_file:
        ...
        for script_config in PYTHON_SCRIPT_NAME:
            ...
            if not script_path:
                try:
                    from souren_config import BASE_DIR
                    base_dir_path = os.path.join(BASE_DIR, script_name)
                    if os.path.exists(base_dir_path):
                        script_path = base_dir_path
                        print(f"  ✅ 在BASE_DIR中找到脚本: {os.path.basename(script_name)}")
                except ImportError:
                    pass
            ...
            if script_path:
                ...
                print(f"     在以下位置查找:")
                print(f"     - {os.path.join(current_dir, 'save_scv', script_name)}")
                print(f"     - {os.path.join(script_dir, 'save_scv', script_name)}")
                print(f"     - {os.path.join(current_dir, script_name)}")
                print(f"     - {os.path.join(script_dir, script_name)}")
    
    if not script_configs:
        print(f"❌ 无法找到可用的Python脚本文件")
        print(f"💡 请尝试以下方法:")
        print(f"   1. 使用 --py-file 参数指定文件路径")
        print(f"   2. 将Python文件放在以下位置之一:")
        print(f"      - save_scv目录: {os.path.join(current_dir, 'save_scv')}")
        print(f"      - 当前目录: {current_dir}")
        print(f"      - 脚本同目录: {script_dir}")
        
        save_scv_dir = os.path.join(current_dir, "save_scv")
        if os.path.exists(save_scv_dir):
            print(f"\n📁 save_scv目录中的Python文件:")
            try:
                save_scv_files = [f for f in os.listdir(save_scv_dir) if f.endswith('.py')]
                if save_scv_files:
                    for f in save_scv_files:
                        print(f"   - {f}")
                else:
                    print("   (无)")
            ...
            print(f"\n⚠️ save_scv目录不存在: {save_scv_dir}")
        
        print(f"\n📁 当前目录中的Python文件:")
        current_py_files = [f for f in os.listdir(current_dir) if f.endswith('.py')]
        if current_py_files:
